...
def sorted_by_bugreport(data, key="timestamp"):
    ...
    OrderedDict
        Input data, sorted.
    """
    return OrderedDict(sorted(data.items(), key=lambda t: int(t[1]["bug_report"][key])))
...
def sorted_by_commit(data, key="timestamp"):
    ...
    """
    return OrderedDict(
        sorted(data.items(), key=lambda t: int(t[1]["commit"]["metadata"][key]))
    )
...
def list_of_bugfixes_to_storage(data, commits):
    ...
    return bitmap_to_bytes(
        list_of_ids_to_bitmap(bugfix_to_idx(data, commits), len(data))
    )
