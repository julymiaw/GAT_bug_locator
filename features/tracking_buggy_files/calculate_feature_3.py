...
def calculate_feature_3(data_prefix, bug_report_id, bug_report_full_sha, bug_reports):
    ...
    current_bug_report_summary_index = pickle.loads(
        bug_report_index_collection[bug_report_id[0:7]]
    )["summary"]
    ...
    if "views" in current_bug_report and "bug_fixing" in current_bug_report["views"]:
        ...
        for sha in shas:
            ...
            if current_file_name in bug_fixing_view:
                ...
                bug_report_history = combine(
                    related_bug_reports, data, bug_report_index_collection
                )
            else:
    ...
    sparse.save_npz(
        data_prefix + "_" + bug_report_id[0:7] + "_feature_3_data",
        feature_3_tf_idf_data,
    )
    with open(
        data_prefix + "_" + bug_report_id[0:7] + "_feature_3_index_lookup", "w"
    ) as outfile:
...
def combine(related_bug_reports, data, bug_report_index_collection):
    indexes = list(
        map(
            lambda bug_report_id: get_bug_report_summary_index(
                bug_report_id, bug_report_index_collection
            ),
            related_bug_reports,
        )
    )
