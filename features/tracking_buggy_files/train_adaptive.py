...
class Adaptive_Process(object):
    ...
    def __init__(self):
        ...
        # 评分方法（目前仅标准评分）
        self.score_methods = [normal_score]
        # endregion
        ...
        self.cut_method = None  # 当前选择的特征筛选方法（函数引用）
        self.score_method = None  # 当前选择的评分方法（函数引用）
        self.columns = None  # 当前使用的特征列名列表
        # endregion
    ...
    def compute_weights(self, df, columns):
        ...
        参数：
            df (pd.DataFrame): 训练数据集，包含特征和used_in_fix标签
            columns (list): 特征列名列表
    ...
    def adapt_process(self, df, columns):
        ...
        results = Parallel(n_jobs=-1)(
            delayed(self._train)(
                df, columns, w_weights, score_method, reg_model, cut_method
            )
    ...
    def _train(
        self, df: pd.DataFrame, columns, weights, score_method, reg_model, cut_method
    ):
        """
        单配置评估内部方法
...
def cut_fit_predict(
    df, df_test, columns, score, score_fixed, cut_method, reg_model: SGDRegressor
):
    """
    特征筛选-训练-预测流水线
...
def size_selectf_only_fixes_p(df, score, perc=0.25, smallest=True, largest=False):
    ...
    参数：
        df (pd.DataFrame): 包含used_in_fix列的数据
        score (np.ndarray): 特征得分向量
        perc (float): 扩展百分比（相对于修复样本数）
        smallest (bool): 是否包含得分最低的负样本
        largest (bool): 是否包含得分最高的负样本

    返回：
        pd.Series: 布尔序列指示所选样本
    """
...
def _process(
    ptemplate: Adaptive_Process, fold_training: pd.DataFrame, fold_testing: pd.DataFrame
):
...
def process(
    ptemplate: Adaptive_Process, fold_number, fold_testing, fold_training, file_prefix
):
    results_list = []
    ...
    with open(
        file_prefix + "_" + ptemplate.name + "_training_time_" + results_timestamp, "w"
    ) as time_file:
        json.dump(training_time, time_file)
    with open(
        file_prefix + "_" + ptemplate.name + "_prescoring_log_" + results_timestamp, "w"
    ) as prescoring_log_file:
        json.dump(prescoring_log, prescoring_log_file)
    with open(
        file_prefix + "_" + ptemplate.name + "_regression_log_" + results_timestamp, "w"
...
def weights_on_df(method, df, columns):
    ...
    参数：
        method (function): 权重计算方法（如weights_chi2）
        df (pd.DataFrame): 当前折训练数据
        columns (list): 特征列名列表
...
def eval_weights(m_name, weights, df, columns):
    ...
    参数：
        m_name (str): 权重方法名称
        weights (np.ndarray): 已计算的权重向量
        df (pd.DataFrame): 验证集数据
        columns (list): 特征列名列表
...
def evaluate_fold(df: pd.DataFrame, Y):
    """
    评估预测结果的MAP指标

    参数：
        df (pd.DataFrame): 待评估数据集（需包含used_in_fix列）
        Y (np.ndarray): 预测得分向量

    返回：
        float: 平均精度均值（Mean Average Precision）
...
def main():
    ...
    # 根据参数决定是否添加特征37和38
    if args.max:
        feature_columns.append("f37")
    if args.mean:
        feature_columns.append("f38")

    file_prefix = args.file_prefix
    fold_number, fold_testing, fold_training = load("data_memmap", mmap_mode="r")

    # file_prefix = "aspectj"
    # fold_number, fold_testing, fold_training = load(
    #     "joblib_memmap_aspectj/data_memmap", mmap_mode="r"
    # )
    ...
    for m in models:
        results.append(
            process(m, fold_number, fold_testing, fold_training, file_prefix)
        )
