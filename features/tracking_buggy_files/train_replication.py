...
def load_fold_number(file_prefix):
    with open(file_prefix + "_fold_info.json", "r") as f:
...
def process(file_prefix):
    ...
    for k in range(fold_number + 1):
        fold_training[k] = pd.read_pickle(
            file_prefix + "_normalized_training_fold_" + str(k)
        )
        fold_testing[k] = pd.read_pickle(
            file_prefix + "_normalized_testing_fold_" + str(k)
        )
    ...
    print(
        "grid search data used in fix equal 1.0",
        grid_search_data_training["used_in_fix"][
            (grid_search_data_training["used_in_fix"] == 1.0)
        ].count(),
    )
    print(
        "grid search data used in fix equal 0.0",
        grid_search_data_training["used_in_fix"][
            (grid_search_data_training["used_in_fix"] == 0.0)
        ].count(),
    )
    grid_search_data_null_columns = grid_search_data_training.columns[
        grid_search_data_training.isnull().any()
    ]
    print("grid search data null columns", grid_search_data_null_columns)
    # exit(0)
    k_range = range(1, 21)
    c = run_grid_search(
        grid_search_data_training, grid_search_data_testing, file_prefix, k_range
    )

    evaluate_algorithm(
        c, fold_training, fold_testing, fold_number, file_prefix, k_range
    )
...
def evaluate_algorithm(
    ...
    for i in range(fold_number):
        ...
        current_training_file = save_svm_rank_data(
            fold_training[i], file_prefix + "_fold_training_" + str(i)
        )
        ...
        accuracy_at_k, mean_average_precision, mean_reciprocal_rank = (
            check_average_precision(testing_df.copy(), weights, k_range)
        )
        for k in k_range:
...
def run_grid_search(training, testing, file_prefix, k_range):
    ...
    data_filename = save_svm_rank_data(
        svm_training, file_prefix + "_grid_search_training"
    )
    ...
    selected_c = None
    for c in c_range:
        mean_average_precision = train_svm_for_grid_search(
            c, data_filename, file_prefix, svm_verification.copy(), k_range
        )
...
def train_svm_for_grid_search(c, data_filename, file_prefix, svm_verification, k_range):
    ...
    accuracy_at_k, mean_average_precision, mean_reciprocal_rank = (
        check_average_precision(svm_verification, weights, k_range)
    )
