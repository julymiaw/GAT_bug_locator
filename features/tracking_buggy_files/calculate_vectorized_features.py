...
def process(bug_reports, data_prefix, bug_report_file_path):
    ...
    with Pool(num_threads, maxtasksperchild=1) as pool:
        if USE_TQDM:
            r = list(tqdm(pool.imap(_f, work), total=len(work)))
        else:
            r = list(pool.imap(_f, work))
    print("r", len(r))
...
def process_bug_report(
    data_prefix, bug_report_full_sha, bug_report_file_path, max_frequency
):
    ...
    vectorized_data = sparse.load_npz(
        data_prefix + "_" + bug_report_id + "_tf_idf_data.npz"
    )
    with open(
        data_prefix + "_" + bug_report_id + "_tf_idf_index_lookup", "r"
    ) as index_lookup_file:
    ...
    (vectorized_report, vectorized_summary, vectorized_description) = load_bug_report(
        vectorized_data, lookups, bug_report_id
    )

    ast_cache_collection = UnQLite(
        data_prefix + "_ast_cache_collection_db", flags=0x00000100 | 0x00000001
    )
    bug_reports = load_bug_reports(bug_report_file_path)
    if bug_report_id in bug_reports:
        current_bug_report_summary = bug_reports[bug_report_id]["bug_report"]["summary"]
    else:
        current_bug_report_summary = retrieve_summary(bug_reports, bug_report_full_sha)[
            "bug_report"
        ]["summary"]

    feature_3_data = sparse.load_npz(
        data_prefix + "_" + bug_report_id + "_feature_3_data.npz"
    )
    with open(
        data_prefix + "_" + bug_report_id + "_feature_3_index_lookup", "r"
    ) as feature_3_file:
        feature_3_file_lookup = json.load(feature_3_file)

    graph_data = sparse.load_npz(
        data_prefix + "_" + bug_report_id + "_graph_features_data.npz"
    ).tocsr()
    with open(
        data_prefix + "_" + bug_report_id + "_graph_features_index_lookup", "r"
    ) as graph_lookup_file:
        graph_lookup = json.load(graph_lookup_file)

    features_5_6_data = sparse.load_npz(
        data_prefix + "_" + bug_report_id[0:7] + "_features_5_6_data.npz"
    ).tocsr()
    with open(
        data_prefix + "_" + bug_report_id[0:7] + "_features_5_6_index_lookup", "r"
    ) as feaures_5_6_lookup_file:
        features_5_6_lookup = json.load(feaures_5_6_lookup_file)

    if bug_report_id in bug_reports:
        fixed_filenames = bug_reports[bug_report_id[0:7]]["commit"]["diff"].keys()
    else:
        fixed_filenames = retrieve_summary(bug_reports, bug_report_full_sha)["commit"][
            "diff"
        ].keys()
    ...
    for current_file_sha in shas:
        ...
        f1 = feature_1(
            ...
            vectorized_data,
            source_index,
            method_source_start_index,
            method_source_end_index,
        )
        f2 = feature_2(
            enriched_report, enriched_api_data, enriched_api_indexes, current_file_sha
        )
        f3 = feature_3(feature_3_data, feature_3_file_lookup, current_file_sha)
        f4 = feature_4(
            current_bug_report_summary, ast_cache_collection, current_file_sha
        )
        ...
        f7 = feature_sim(
            vectorized_summary, vectorized_data, class_start_index, class_end_index
        )
        f8 = feature_sim(
            vectorized_summary,
            vectorized_data,
            method_names_start_index,
            method_names_end_index,
        )
        f9 = feature_sim(
            vectorized_summary,
            vectorized_data,
        ...
        f10 = feature_sim(
            vectorized_summary, vectorized_data, comment_start_index, comment_end_index
        )

        f11 = feature_sim(
            vectorized_description, vectorized_data, class_start_index, class_end_index
        )
        f12 = feature_sim(
...
def load_enriched_api(data_prefix, commit):
    ...
    data = sparse.load_npz(
        data_prefix + "_" + bug_report_id + "_tfidf_enriched_api.npz"
    )
    with open(
        data_prefix + "_" + bug_report_id + "_partial_enriched_api_index_lookup", "r"
    ) as lookup_file:
