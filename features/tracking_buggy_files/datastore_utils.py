...
class NestedDictStore:
    ...
    def set_feature(self, commit, pathname, feature, value):
        ...
        self.data[commit]["views"][self.category][pathname][feature] = deepcopy(value)
...
class DataFrameHDF5:
    ...
    def _flush_maybe(self, commit):
        ...
        if self.commit is None:
            ...
            if len(self.df_br > 0):
                self.store["g" + self.commit + "/br"] = self.df_br.to_sparse(
                    fill_value=0
                )
                # DEBUG
    ...
    def set_feature(self, commit, pathname, feature, value):
        ...
        if feature not in self.df.columns:
            ...
            self.df.append(pd.DataFrame.from_dict({feature: {pathname: value}}))
        else:
            # modify scalar value
