...
def convert_tf_idf(data_prefix, bug_report_full_sha):
    ...
    ast_index_collection = UnQLite(
        data_prefix + "_ast_index_collection_index_db", flags=0x00000100 | 0x00000001
    )
    ...
    with open(
        data_prefix + "_" + bug_report_id + "_tf_idf_index_lookup", "w"
    ) as outfile:
...
def extract_ast(data, current_indexes):
    ...
    methods_rows = data[
        current_indexes["methodsStart"] : current_indexes["methodsEnd"] + 1, :
    ]
    ...
    class_names_rows = data[
        current_indexes["classNamesStart"] : current_indexes["classNamesEnd"] + 1, :
    ]
    ...
    method_names_rows = data[
        current_indexes["methodNamesStart"] : current_indexes["methodNamesEnd"] + 1, :
    ]
    ...
    variable_names_rows = data[
        current_indexes["variableNamesStart"] : current_indexes["variableNamesEnd"] + 1,
        :,
    ]
    ...
    comments_rows = data[
        current_indexes["commentsStart"] : current_indexes["commentsEnd"] + 1, :
    ]
