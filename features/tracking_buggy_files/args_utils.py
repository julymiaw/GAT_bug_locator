...
def add_arguments_and_usage(parser, repo_required=False):
    ...
    """
    parser.add_argument(
        "json_file", metavar="<json-file>", help="data file in JSON format to process"
    )
...
def read_json(filename, preserve_order=False):
    ...
    with safeopen(filename) as datafile:
        ...
        if filename != "-":
            print(
                "Loading %s data file: '%s'"
                % (sizeof_fmt(os.path.getsize(filename)), filename),
                file=sys.stderr,
            )
        else:
            print("Reading data from standard input", file=sys.stderr)
        # read file
        if preserve_order:
...
def print_json(data, args):
    ...
    print(  # "Saving/printing %s data set with %d elements" %
        ...
        "Saving/printing data set with %d elements" % (len(data)),
        file=sys.stderr,
    )
    # write output/file
    # write output/file
    with open(output_file, "w") if output_file != sys.stdout else output_file as f:
        json.dump(
            data,
            f,
            indent=indent_arg,
            separators=(",", ": ") if indent_arg is not None else None,
        )
    # progress info
