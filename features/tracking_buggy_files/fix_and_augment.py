...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Normalize, fix and augment bug + repo data" + ", augment repo"
    )
    ...
    parser.add_argument(
        ...
        nargs="?",
        const="bug",
        help="sorting by bug or commit timestamp" + " [default '%(const)s']",
    )
    parser.add_argument(
        "--sort-keys", action="store_true", help="sort (sub)keys in predefined order"
    )
    parser.add_argument(
        "-t",
        "--add-tags",
...
def process_data(data, repo, sort="bug", sort_keys=False, add_tags=False, **kwargs):
    ...
    **kwargs
    ...
    if sort and sort in ("bug", "commit"):
        print("Sorting data by %s timestamp" % sort, file=sys.stderr)
    if sort == "bug":
    ...
    if sort_keys:
        print("Sorting keys in data in predefined order", file=sys.stderr)
        data_sort_keys(data)
...
def fix_commit_metadata(data):
    ...
    for commit in tqdm(data):
        ...
        data[commit]["commit"]["metadata"]["timestamp"] = datetime_to_timestamp(
            convert_commit_date(data[commit]["commit"]["metadata"]["date"])
        )

    print(
        '%d / %d skipped: had already "timestamp" field in commit metadata'
        % (n_skipped, len(data)),
        file=sys.stderr,
    )
...
def find_preceding_commits(data, repo):
    ...
    for commit in tqdm(data):
        if field_name in data[commit]["bug_report"]:
            n_skipped = n_skipped + 1
            continue
        data[commit]["bug_report"][field_name] = git_utils.find_commit_by_timestamp(
            repo,
    ...
    print(
        '%d / %d skipped: had already "%s" field for bug report'
...
def tag_bugfixing_commits(repo, data):
    ...
    for commit in tqdm(data):
        ...
        git_utils.create_tag(repo, bugtag, data[commit]["commit"]["metadata"]["sha"])

    # info about process
    print("%d / %d skipped: already tagged" % (n_skipped, len(data)), file=sys.stderr)
