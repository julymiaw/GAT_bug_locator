...
def process_bug_report(data_prefix, bug_report_id, bug_report_file_path):
    ...
    (vectorized_report, vectorized_summary, vectorized_description) = load_bug_report(
        vectorized_data, bug_report_index_lookup, bug_report_id
    )

    enriched_report = enriched_api_data[
        enriched_api_bug_reports_lookup[bug_report_id], :
    ]
    ...
    for file_index in file_index_lookup:
        ...
        f2 = feature_2(
            enriched_report, enriched_api_data, enriched_api_lookup, file_index
        )
