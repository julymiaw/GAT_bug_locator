...
removed = "!\"#%&'()*+,-./:;<=>?@[\]^_`{|}~1234567890"
utf_translate_table = dict((ord(char), " ") for char in removed)
...
def tokenize(text, stemmer):
    ...
    return Counter(
        [stemmer.stem(token) for token in all_tokens if token.lower() not in stop_words]
    )
...
def vectorize(ast_cache, bug_reports, data_prefix):
    ...
    if USE_TQDM:
        iterator = tqdm(bug_reports, desc="Processing bug reports", leave=True)
    else:
        iterator = bug_reports
    for bug_report_id in iterator:
