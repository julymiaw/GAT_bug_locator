...
from git_utils import retrieve_changes, retrieve_files, retrieve_changes_status
from date_utils import convert_commit_date
import args_utils
from datastore_utils import NestedDictStore, select_datastore_backend
from misc_utils import cmp_char

from fix_and_augment import fix_commit_metadata, sorted_by_bugreport, sorted_by_commit
from dataset_utils import list_of_bugfixes_to_storage
...
def main():
    ...
    parser.add_argument(
        "--count-files", action="store_true", help="count files in each bugfix revision"
    )
    parser.add_argument(
        "--br",
        default="list",
        choices=["list", "sparse", "bitmap", "none"],
        help="storage format for br(r,s)" + " [default '%(default)s']",
    )
    ...
    if isinstance(store, NestedDictStore):
        ...
        if br_format != "list" and br_format != "none":
            print(
                "--br=%s not supported for JSON, switching to 'list'" % (br_format,),
                file=sys.stderr,
            )
            br_format = "list"
    ...
    print("Repository: %s" % (repo_path,), file=sys.stderr)
...
def process_stats(stats, output_file):
    ...
    print(pd.DataFrame(df_sum.sum(), columns=["sum"]), file=sys.stderr)
    ...
    if output_file != sys.stdout:
        ...
        with open(stats_filename, "w") as stats_file:
            df.drop(labels=["fixes", "timestamp"], axis=1).sort_index(axis=1).describe(
                percentiles=[0.1, 0.25, 0.5, 0.75, 0.9]
            ).to_csv(stats_file)
            print("", file=stats_file)
            pd.DataFrame(df_sum.sum(), columns=["sum"]).T.to_csv(stats_file)

        print(
            "Saved description of stats to  '%s' (%s)"
            % (stats_filename, args_utils.sizeof_fmt(os.path.getsize(stats_filename))),
            file=sys.stderr,
        )
...
def process_data(store, data, repo, count_files=False):
    ...
    br_remaining = sorted(
        data.keys(), key=lambda c: int(data[c]["commit"]["metadata"]["timestamp"])
    )
    ...
    data_iter = tqdm(data) if USE_TQDM else data
    for commit in data_iter:
        # DEBUG
        ...
        init_bugfix_changes_stats(stats, data, commit, changes=fixstatus)
        create_commit_info_stats(stats, data, commit)

        # debugging strange patterns in the dataset
        if int(data[commit]["commit"]["metadata"]["timestamp"]) < int(
            data[commit]["bug_report"]["timestamp"]
        ):
        ...
        stats[commit]["has_prev_in_new"] = len(br_new) > 0 and prev_commit in br_new
        ...
        for br_commit in br_new:
            ...
            if br_data and prev_br_commit is not None:
                ...
                diffstatus = retrieve_changes_status(
                    repo, commit=br_commit_full + "^", prev=prev_br_commit_full + "^"
                )
                changes_stats = br_translate(br_data, changes=diffstatus)
                # update bugfix commit stats
                if br_commit not in stats:
                    # debugging strange patterns in the dataset
                    n_suspicious += 1
                    if int(data[commit]["bug_report"]["timestamp"]) == int(
                        data[br_commit]["bug_report"]["timestamp"]
                    ):
                    ...
                    if max_warnings > 0 and n_suspicious < max_warnings:
                        print(
                            "%s* commit=%s [%d/id=%d] timestamps: bug=%d %s commit=%d"
                            % (
                                cmp_char(
                        ...
                        print(
                            "br_commit=%s [%d/id=%d] timestamps: bug=%d %s commit=%d"
                            % (
                                br_commit,
                                commit_idx[br_commit],
                                int(data[br_commit]["bug_report"]["id"]),
                                int(data[br_commit]["bug_report"]["timestamp"]),
                                cmp_char(
                                    int(data[br_commit]["bug_report"]["timestamp"]),
                                    int(
                                        data[br_commit]["commit"]["metadata"][
                                            "timestamp"
                                        ]
                                    ),
                                ),
            ...
            changes_effective = br_add_bugfix(
                br_data, bugfix=br_commit, changes=fixstatus
            )
        ...
        br_commit_full = data[br_commit]["commit"]["metadata"]["sha"]
        diffstatus = retrieve_changes_status(
            repo, commit=commit_full + "^", prev=br_commit_full + "^"
        )
        ...
        stats_br = br_compute_frequency_and_recency(store, br_curr, data, commit)
    ...
    if n_suspicious > 0:
        print(
            "%d/%d suspicious bug reports, with br_commit not in stats"
            % (n_suspicious, n_total),
            file=sys.stderr,
        )
        if max_warnings > 0:
            print("- supressed warnings after %d" % max_warnings, file=sys.stderr)
    if n_bug_time_collisions > 0:
        print(
            "%d/%d occurences where two bug reports had the same timestamp"
            % (n_bug_time_collisions, n_total),
            file=sys.stderr,
        )
    if n_fix_before_report > 0:
        print(
            "%d/%d bug reports was fixed before bug report creation date"
            % (n_fix_before_report, n_total),
            file=sys.stderr,
        )
    ...
    if count_files:
        ...
        for commit in data_iter:
            ...
            stats[commit]["n_files_Java"] = len(
                [f for f in files if f.endswith(".java")]
            )
        ...
        print(
            "number of files in repository at last bugfix %s: %d"
    ...
    for path in bug_fixing_data:
        ...
        if br_format == "list":
            ...
            store.set_feature(
                ...
                path,
                "br",
                list_of_bugfixes_to_storage(data, bug_fixing_data[path]),
            )
        elif br_format != "none":
    ...
    stats[commit].update(
        {
            ...
            "n_effective": len(
                [pair for pair, status in changes.items() if status != "D"]
            ),
            "n_renames": len(
                [pair for pair, status in changes.items() if status == "R"]
            ),
            "n_deletions": len(
                [pair for pair, status in changes.items() if status == "D"]
            ),
            "n_additions": len(
                [pair for pair, status in changes.items() if status == "A"]
            ),
        }
    )
