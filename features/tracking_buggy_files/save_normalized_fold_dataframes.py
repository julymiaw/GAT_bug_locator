...
def process(bug_reports, file_prefix):
    ...
    fold_index = 0
    if USE_TQDM:
        iterator = tqdm(sorted_commits)
    else:
        iterator = sorted_commits
    for commit, _ in iterator:
        ...
        irrelevant = df[(df["used_in_fix"] == 0)].nlargest(
            number_of_irrelevant_files, "f2"
        )
    ...
    save_normalized_data(
        file_prefix, fold_number, fold_testing, fold_training, max_dict, min_dict
    )
...
def save_normalized_data(
    ...
    for k, df in fold_training.items():
        ...
        min_df = min_dict[k]
        max_df = max_dict[k]
        normalized_df = (df.drop("used_in_fix", axis=1) - min_df.min()) / (
            max_df.max() - min_df.min()
        )
    ...
    print(info)
    with open(file_prefix + "_fold_info.json", "w") as info_file:
