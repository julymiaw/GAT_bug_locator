...
def add_types_source_to_bug_report_data(data, data_prefix, class_name_lookup, ast_sha):
    ...
    types = UnQLite(
        data_prefix + "_ast_types_collection_index_db", flags=0x00000100 | 0x00000001
    )
    ...
    for method_types in types_per_method:
        ...
        supertypes_shas_per_type = [
            set(find_types_shas(types, class_name_lookup, s)) for s in method_type_shas
        ]
...
def extract_enriched_api(data_prefix, bug_report_full_sha):
    ...
    with open(
        data_prefix + "_" + bug_report_id + "_partial_enriched_api_index_lookup", "w"
    ) as outfile:
    ...
    sparse.save_npz(
        data_prefix + "_" + bug_report_id + "_tfidf_enriched_api", tf_idf_data
    )
