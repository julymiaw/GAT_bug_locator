...
def fix_index(fold_testing):
    ...
    for fold in fold_testing:
        ...
        fold_testing[fold].index.set_levels(
            range(fold * max_level_0, (fold + 1) * max_level_0), level=0, inplace=True
        )
        fold_testing[fold].index.set_levels(
            range(fold * max_level_1, (fold + 1) * max_level_1), level=1, inplace=True
        )
...
def main():
    ...
    cwd = os.getcwd()
    folder = os.path.join(cwd, "../joblib_memmap_" + file_prefix + "_flim")
