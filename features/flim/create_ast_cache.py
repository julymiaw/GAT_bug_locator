...
def _process_notes(note, note_not_tokenize, repository_path):
    ...
    (note_content_sha_not_tokenize, note_object_sha_not_tokenize) = (
        note_not_tokenize.split(" ")
    )

    assert note_object_sha == note_object_sha_not_tokenize

    note_content = cat_file_blob(repository_path, note_content_sha)
    note_content_not_tokenize = cat_file_blob(
        repository_path, note_content_sha_not_tokenize
    )
    ast_extraction_result = json.loads(note_content)
    ast_extraction_result_not_tokenize = json.loads(note_content_not_tokenize)
    for key in ast_extraction_result_not_tokenize:
        if key not in ast_extraction_result:
            ast_extraction_result[key] = ast_extraction_result_not_tokenize[key]
...
def prepare_ast_cache(repository_path):
    ...
    with Pool(cpu_count()) as pool:  # 统一使用进程池
        ast_cache = dict(pool.map(_f, work))  # 统一处理结果
