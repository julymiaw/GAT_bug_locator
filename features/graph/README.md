...
2. GAT模型(图注意力网络)结果
    ...
    | 属性        | 分数   |     | 属性        | 分数   |
    | ----------- | ------ | --- | ----------- | ------ |
    | Accuracy@1  | 0.4385 |     | Accuracy@11 | 0.7101 |
    | Accuracy@2  | 0.5248 |     | Accuracy@12 | 0.7229 |
    | Accuracy@3  | 0.5908 |     | Accuracy@13 | 0.7394 |
    | Accuracy@4  | 0.6220 |     | Accuracy@14 | 0.7450 |
    | Accuracy@5  | 0.6495 |     | Accuracy@15 | 0.7505 |
    | Accuracy@6  | 0.6606 |     | Accuracy@16 | 0.7560 |
    | Accuracy@7  | 0.6734 |     | Accuracy@17 | 0.7596 |
    | Accuracy@8  | 0.6862 |     | Accuracy@18 | 0.7615 |
    | Accuracy@9  | 0.6954 |     | Accuracy@19 | 0.7633 |
    | Accuracy@10 | 0.7046 |     | Accuracy@20 | 0.7651 |
    | MAP         | 0.5272 |     | MRR         | 0.5219 |
...
3. 模型性能对比分析
    ...
    | GAT模型      | 0.5272 | 0.5219 | 0.4385     | 0.6495     | 0.7046      | 0.7651      |
    | 提升百分比   | -0.06% | -0.25% | 2.12%      | 3.19%      | -0.25%      | 1.70%       |

4. AspectJ与Tomcat数据集比较分析

    各模型在两个数据集上的表现差异显著。相较于AspectJ数据集，Tomcat数据集上的模型普遍表现出更高的准确率：

    | 数据集  | 模型         | MAP    | MRR    | Accuracy@1 | Accuracy@5 |
    | ------- | ------------ | ------ | ------ | ---------- | ---------- |
    | AspectJ | SGDRegressor | 0.2665 | 0.3175 | 0.1842     | 0.4605     |
    | AspectJ | GAT模型      | 0.2961 | 0.3439 | 0.2237     | 0.4868     |
    | Tomcat  | SGDRegressor | 0.5275 | 0.5232 | 0.4294     | 0.6294     |
    | Tomcat  | GAT模型      | 0.5272 | 0.5219 | 0.4385     | 0.6495     |

    在Tomcat数据集上，GAT模型与SGDRegressor表现相近，仅在部分指标上有微小改进。而在AspectJ数据集上，GAT模型相比基准模型有显著提升(MAP提升11.1%，Accuracy@1提升21.4%)。

    这一差异可能与两个数据集的图结构特性有关：Tomcat数据集中消息传递路径更长(平均跳数3.36 vs 1.93)，节点在2-3跳内分布更均匀，而SGDRegressor已能很好地捕获这些特征。
...
总结来看，GAT模型在AspectJ数据集上的出色表现验证了基于图结构和消息传递机制在缺陷文件定位任务中的有效性，并为未来的研究提供了有价值的参考。
