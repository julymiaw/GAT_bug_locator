...
1. 模型性能比较（交叉验证得分）
    ...
    | 模型类别     | 平均得分 | 标准差 | 最低得分 | 最高得分 | 模型数量 |
    | ------------ | -------- | ------ | -------- | -------- | -------- |
    | baseline_mlp | 0.5102   | 0.0211 | 0.4893   | 0.5312   | 2        |
    | gat_realedge | 0.5138   | 0.0195 | 0.4912   | 0.5368   | 4        |
    | gat_selfloop | 0.4986   | 0.0287 | 0.4613   | 0.5207   | 4        |

2. 最佳模型分析

    - **最佳模型**: GATRegressor_MSE_32_l2_4_a0.0001_dr0.2_lr0.001
    - **类型**: GAT模型（使用真实依赖边）
    - **测试MAP得分**: 0.5272（与前面的评估结果一致）
...
4. 主要发现
    ...
    1. **模型性能差异较小**：在Tomcat数据集上，GAT模型(MAP=0.5138)与基准MLP模型(MAP=0.5102)性能相近，提升较为有限，仅约0.7%。

    2. **真实边仍有价值**：使用真实依赖边的GAT模型(MAP=0.5138)略优于仅使用自环的GAT模型(MAP=0.4986)，但优势不如在AspectJ数据集上明显。
